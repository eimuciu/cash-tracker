...
export default function IncomeIconsSetting({ closeModal }: Props) {
  ...
  const { settings, addIncomeIconSettings }: any = useDataContext();

  useEffect(() => {
    setIconsList(settings.incomeIcons);
  }, [settings.incomeIcons]);
  ...
  const onIconSettingConfirm = () => {
    addIncomeIconSettings(iconsList);
    closeModal();
  };
